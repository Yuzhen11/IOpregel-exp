...
class IDRecVertex:public Vertex<VertexID, double, SPEdge, SPEdge> //MsgT == KeyT
{
	public:
		virtual void compute(vector<SPEdge>& msgs, vector<SPEdge>& edges)
		{
			if(step_num()==1)
			{
				//send requests to out-nbs
				for(int i=0; i<edges.size(); i++) send_message(edges[i].nb, SPEdge{edges[i].len, id});
			}
			else if(step_num()==2)
			{
				//respond
				for(int i=0; i<msgs.size(); i++) send_message(msgs[i].nb, SPEdge{msgs[i].len, recoded_id()});
			}
			else
			{
				//append to eout
...
int main(int argc, char* argv[])
{
	...
	string local_root = "/home/yanda/tmp/iopregel/webuk";
	//runner.run_recode(local_root, argc, argv); //local Load
	//string hdfs_inpath = "/yuzhen/toy_weighted";
