...
class IDRecVertex:public Vertex<VertexID, VertexID, VertexID, VertexID> //MsgT == KeyT
{
	public:
		virtual void compute(vector<VertexID>& msgs, vector<VertexID>& edges)
		{
			...
			{
				//respond
				for(int i=0; i<edges.size(); i++) send_message(edges[i], recoded_id());
			}
			else
			{
				//append to eout
