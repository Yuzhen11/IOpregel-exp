...
struct PagerankProgramInmem : public GraphChiProgram<VertexDataType, EdgeDataType> {
    ...
    void update(graphchi_vertex<VertexDataType, EdgeDataType> &v, graphchi_context &ginfo) {
        if (ginfo.iteration == 0) {
            ...
            for (int i = 0; i < v.num_inedges(); ++ i) {
                std::cout << v.inedge(i)->vertexid << " ";
            }
            std::cout << std::endl;
            for (int i = 0; i < v.num_outedges(); ++ i) {
                std::cout << v.outedge(i)->vertexid << " ";
            }
            std::cout << std::endl;
        }
    }
...
int main(int argc, const char ** argv) {
    ...
    if (inmemmode) {
        ...
        std::cout << "Running Pagerank by holding vertices in-memory mode!" << std::endl;
    } else {
        PagerankProgram program;
        engine.run(program, niters);
        std::cout << "Running Pagerank with EM mode!" << std::endl;
    }
